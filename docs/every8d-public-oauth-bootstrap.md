...
An accepted POST calls the existing start operation once, reuses the narrow `wincrm_every8d_oauth_binding` cookie (`Secure`, `HttpOnly`, `SameSite=Lax`, and `Path=/oauth/every8d-connect`, with no `Domain`), and returns HTTP 303 to the authorization URL returned by that operation. The route accepts no tenant, company, application, provider, installation, redirect, state, or binding input and cannot act as an open redirect.
