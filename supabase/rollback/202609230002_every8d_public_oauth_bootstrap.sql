...
begin
  if exists (select 1 from public.ghl_marketplace_oauth_bootstraps)
    or exists (select 1 from public.ghl_marketplace_app_version_registrations)
    or exists (select 1 from public.ghl_marketplace_installations
      where latest_lifecycle_version_id is not null) then
    raise exception 'public OAuth bootstrap rollback refused: feature-owned evidence exists'
      using errcode = '23514';
  end if;
end;
$$;
...
drop function public.accept_every8d_public_oauth_callback_v1(uuid,text,text,text,text,bytea,text);
drop function public.inspect_every8d_public_oauth_callback_v1(text,text,text,text);
drop function public.create_every8d_public_oauth_bootstrap_v1(text,text,text,text,text,text,text,text,integer);
